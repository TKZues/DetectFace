                  ...
                  children: [
                    ...
                    GridView.builder(
                      ...
                      itemBuilder: (context, index) {
                        String title = repository.listDiemchuyencan[index].className ?? "";
                        int totalCheckIns = repository.listDiemchuyencan[index].totalCheckIns ?? 0;
                        int totalAbsences = repository.listDiemchuyencan[index].totalAbsences ?? 0;
                        int totalLate = repository.listDiemchuyencan[index].totalLate ?? 0;
                
                        List<AttendanceStatus> weeklyData = [
                          AttendanceStatus('Có mặt', totalCheckIns.toDouble(), Colors.green),
                          AttendanceStatus('Vắng mặt', totalAbsences.toDouble(), Colors.red),
                          AttendanceStatus('Đi muộn', totalLate.toDouble(), Colors.orange),
                        ];
                        ...
                        return SfCircularChart(
                          ...
                          series: <CircularSeries>[
                            PieSeries<AttendanceStatus, String>(
                              ...
                              xValueMapper: (AttendanceStatus status, _) => status.status,
                              yValueMapper: (AttendanceStatus status, _) => status.percentage,
                              dataLabelMapper: (AttendanceStatus status, _) =>
                                  '${status.status}: ${status.percentage}%',
                              pointColorMapper: (AttendanceStatus status, _) => status.color,
                              enableTooltip: true,
                            ),
                          ],
                        );
